...
from models import Simple_LSTM,TransformerEncoder_LSTM_1, TransformerEncoder_LSTM_OrdinalRegressor
...
class ScaniaModuleOrdinalRegression(pl.LightningModule):
    def __init__(self, lr, model_version, problem_type = 'regression', weights=None,**kwargs):
        assert problem_type in ['regression','classification'], 'Only regression or classification are supported for Scania dataset'
        super(ScaniaModuleOrdinalRegression, self).__init__()
        self.problem_type = problem_type
        self.save_hyperparameters()
        self.net = TransformerEncoder_LSTM_OrdinalRegressor(problem_type=self.problem_type,**kwargs)
        self.lr = lr
        ...
        self.loss_fn = CumulativeLinkLoss(class_weights=weights.to('cuda'))
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss_fn(x, y.argmax(dim=len(y.shape)-1))
...
def train_model_with_dataloaders(
    ...
    model = ScaniaModuleOrdinalRegression(
        lr=lr,
        **model_kwargs
    )
    ...
    if problem_type == 'regression':
        checkpoint_callback = pl.callbacks.ModelCheckpoint(
         ...
         checkpoint_callback = pl.callbacks.ModelCheckpoint(
            dirpath=f'./checkpoints/model-{model_version}-only_two_classes',
            monitor='val_loss',
    ...
    trainer = pl.Trainer(
        ...
        devices=1,
        max_epochs=500,
        callbacks=[early_stop_callback, checkpoint_callback, ClipCutpoints()],
...
def main():
    ...
    parser.add_argument('--fc-layer-dim', type=int, default=256)
    ...
    parser.add_argument('--model-version', type=str, default='ordinal_regression', help='model version to train values: ordinal_regression')
    ...
    # for model in ['lstm','version_1','version_2','version_3']:
    for model in ['ordinal_regression']:
